...
def test_valid_char_field(value):
...
def test_valid_email_field(value):
...
def test_valid_phone_field(value):
...
def test_valid_birthday_field(value):
...
def test_valid_gender_field(value):
...
def test_valid_client_ids_field(value):
...
def test_invalid_client_ids_field(value):
    f = ClientIDsField(required=True)
    with pytest.raises(ValueError):
        f.__set__(None, value)
